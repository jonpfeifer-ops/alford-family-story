...
window.AlfordRiverAtlas=class {
 constructor(container,config,geometry,records){
  ...
  const river=geometry.river,region=geometry.region;
  this.regionLayer=svg('g',{class:'border-region'},this.world);
  this.terrain=svg('image',{...Object.fromEntries(['x','y','width','height'].map(k=>[k,region.relief[k]])),href:region.relief.image,preserveAspectRatio:'none',class:'border-relief'},this.regionLayer);
  for(const f of region.counties)svg('path',{d:f.d,class:'border-county'},this.regionLayer);
  for(const f of region.areas)svg('path',{d:f.d,class:f.main?'border-area-main':'border-area-minor','fill-rule':'evenodd'},this.regionLayer);
  for(const f of region.water){if(f.main)svg('path',{d:f.d,class:'border-water-halo'},this.regionLayer);svg('path',{d:f.d,class:f.main?'border-water-main':'border-water-minor'},this.regionLayer);}
  this.regionInset=document.createElement('aside');this.regionInset.className='border-inset';this.regionInset.setAttribute('aria-label','Location on the Louisiana–Mississippi border');const ib=region.insetBox,ik=100/ib[2];this.regionInset.innerHTML=`<svg viewBox="0 0 100 ${ib[3]*ik}" aria-hidden="true"><g transform="matrix(${ik} 0 0 ${ik} ${-ib[0]*ik} ${-ib[1]*ik})">${region.states.map(f=>`<path class="state-${f.name}" d="${f.d}"/>`).join('')}<rect/></g><text x="${(-230000-ib[0])*ik}" y="${(45000-ib[1])*ik}">LA</text><text x="${(25000-ib[0])*ik}" y="${(-210000-ib[1])*ik}">MS</text></svg>`;this.canvas.appendChild(this.regionInset);
  this.regionLabels=region.labels.filter(f=>f.name!=='Osyka').sort((a,b)=>(a.kind==='river'?0:1)-(b.kind==='river'?0:1)).map(f=>({...f,el:this.makeLabel('border-label border-label-'+f.kind,escape(f.name)),dot:f.kind==='town'?svg('circle',{r:2.5,class:'border-town-dot'},this.map):null}));
  ...
  this.border=svg('path',{d:'M-100000 0H100000',class:'river-atlas-border'},this.world);
  ...
  this.claimLeader=svg('path',{class:'border-claim-leader'},this.map);
 ...
 update(id,p){
  ...
  if(this.key!==id){cancelAnimationFrame(this.replayFrame);this.replayFrame=0;this.key=id;this.scene=scene;this.wide=false;this.cameraOverride=null;
   ...
   this.el.querySelector('.atlas-wider').hidden=false;
 ...
 paint(progress){
  ...
  const relief=this.geometry.region.relief;
  const scale=Math.max(Math.min((w-(mobile?35:70))/c[2],(h-(origins?35:mobile?55:65))/c[3]),origins?Math.max(w/relief.width,h/relief.height):0);
  let x=w*.5-c[0]*scale,y=h*.47-c[1]*scale;
  if(origins){x=Math.max(w-(relief.x+relief.width)*scale,Math.min(-relief.x*scale,x));y=Math.max(h-(relief.y+relief.height)*scale,Math.min(-relief.y*scale,y));}this.world.setAttribute('transform',`translate(${x} ${y}) scale(${scale})`);
  const regionalOpacity=clamp((c[2]-8000)/8000);this.regionLayer.style.opacity=regionalOpacity;
  for(const el of this.world.querySelectorAll('.river-atlas-valley,.river-atlas-bank,.river-atlas-water,.river-atlas-light'))el.style.opacity=1-regionalOpacity;
  this.regionInset.hidden=!origins;
  ...
  const occupied=origins?[{x:w-45,y:0,w:45,h:48},{x:w-this.regionInset.offsetWidth-12,y:h-this.regionInset.offsetHeight-12,w:this.regionInset.offsetWidth+12,h:this.regionInset.offsetHeight+12},{x:0,y:h-32,w:100,h:32}]:[{x:w-mini.offsetWidth-(mobile?9:14),y:h-mini.offsetHeight-(mobile?9:13),w:mini.offsetWidth,h:mini.offsetHeight}];
  ...
  this.claimLeader.style.opacity=0;
  this.townLabel.style.opacity=0;this.townDot.style.opacity=origins?1:0;this.distance.style.opacity=0;this.distanceLabel.style.opacity=0;this.northLabel.style.opacity=0;this.southLabel.style.opacity=0;
  if(origins){
   const jl=this.labels.find(l=>l.id==='jacob'),[tx,ty]=project(this.town.point),[jx,jy]=project(jl.point),borderY=project([0,0])[1];
   const lx=parseFloat(jl.el.style.left),ly=parseFloat(jl.el.style.top),ex=Math.max(lx+8,Math.min(lx+jl.el.offsetWidth-8,jx));if(jl.el.style.opacity==='1'){this.claimLeader.setAttribute('d',`M${jx} ${jy}L${ex} ${ly}`);this.claimLeader.style.opacity=1;}
   this.townDot.setAttribute('cx',tx);this.townDot.setAttribute('cy',ty);place(this.townLabel,this.town.point,[-25,-52],true);
   for(const fraction of [.53,.78,.13]){const left=w*fraction-this.northLabel.offsetWidth/2,r={x:left,y:borderY-18,w:Math.max(this.northLabel.offsetWidth,this.southLabel.offsetWidth),h:34};if(occupied.some(o=>r.x<o.x+o.w+7&&r.x+r.w+7>o.x&&r.y<o.y+o.h+5&&r.y+r.h+5>o.y))continue;for(const [el,dy] of [[this.northLabel,-18],[this.southLabel,5]]){el.style.opacity=1;el.style.left=left+'px';el.style.top=(borderY+dy)+'px';}occupied.push(r);break;}
  ...
  for(const f of this.regionLabels){f.el.style.opacity=0;if(f.dot)f.dot.style.opacity=0;if(!origins)continue;
   const offset=f.kind==='river'?[10,f.name==='Tangipahoa River'?18:8]:[7,-16];place(f.el,f.point,offset);
   if(f.dot&&f.el.style.opacity==='1'){const [px,py]=project(f.point);f.dot.setAttribute('cx',px);f.dot.setAttribute('cy',py);f.dot.style.opacity=1;}}
  const regionRect=this.regionInset.querySelector('rect');regionRect.setAttribute('x',-x/scale);regionRect.setAttribute('y',-y/scale);regionRect.setAttribute('width',w/scale);regionRect.setAttribute('height',h/scale);
  const verticalRiver=s.frame!=='claim'&&!this.wide;this.riverLabel.classList.toggle('is-vertical',verticalRiver);
  this.riverLabel.style.opacity=0;if(!origins)place(this.riverLabel,verticalRiver?[2700,3950]:this.wide?[3600,2400]:[3800,1050],verticalRiver?[22,-50]:[-40,-15]);
  ...
  const metres=origins?(c[2]>55000?16093.44:8046.72):this.wide?1609.344:(804.672*scale>115?402.336:804.672);this.el.querySelector('.river-atlas-scale i').style.width=(metres*scale)+'px';this.el.querySelector('.river-atlas-scale span').textContent=origins?(c[2]>55000?'10 miles':'5 miles'):this.wide?'1 mile':metres<500?'¼ mile':'½ mile';
  ...
  this.el.dataset.progress=p.toFixed(3);this.el.dataset.camera=c.map(v=>v.toFixed(1)).join(',');
 }
};
})();
